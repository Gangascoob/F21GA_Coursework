...
void startup()
{
	...
	content.LoadGLTF("assets/car.glb");
...
void update()
{
	if (keyStatus[GLFW_KEY_LEFT]) modelRotation.y += 0.015f;
	if (keyStatus[GLFW_KEY_RIGHT]) modelRotation.y -= 0.015;
	if (keyStatus[GLFW_KEY_UP]) modelRotation.x += 0.015f;
	if (keyStatus[GLFW_KEY_DOWN]) modelRotation.x -= 0.015f;
	

	if (keyStatus[GLFW_KEY_R]) pipeline.ReloadShaders();

//Added camera movement controls
...
void render()
{
	...
	modelMatrix = glm::rotate(modelMatrix, modelRotation.z, glm::vec3(0.0f, 0.0f, 1.0f));	//added to allow zooming in and out
